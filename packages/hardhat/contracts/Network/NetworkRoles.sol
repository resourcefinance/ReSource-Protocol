...
contract NetworkRoles is AccessControlUpgradeable, OwnableUpgradeable, INetworkRoles  {
    ...
    modifier onlyAmbassador() {
        require(hasRole("AMBASSADOR", msg.sender) || hasRole("OPERATOR", msg.sender), "NetworkRoles: Only ambassadors can call");
        _;
    }
    ...
    function inviteMember(address _member) external onlyAmbassador{
        require(!memberAmbassadorInvites[_member][msg.sender], "CreditRequest: Invite already exists");
        memberAmbassadorInvites[_member][msg.sender] = true;
    }

    function acceptAmbassadorInvitation(address _ambassador) external { 
        require(memberAmbassadorInvites[msg.sender][_ambassador], "CreditRequest: Invite does not exist");
    ...
    function getMemberAmbassador(address _member) external override view returns(address) {
        return memberAmbassador[_member];
    }

    function isAmbassador(address _ambassador) external override view returns(bool) {
        return hasRole("AMBASSADOR", _ambassador);
    }

    function isNetworkOperator(address _operator) external override view returns(bool) {
    ...
    function deployMemberWallet(
        ...
        memberAmbassador[newWallet] = msg.sender;
