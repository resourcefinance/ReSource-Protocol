...
contract CreditManager is OwnableUpgradeable, PausableUpgradeable, ICreditManager {
    ...
    modifier onlyOperator() {
        require(
            protocolRoles.isProtocolOperator(msg.sender),
    ...
    modifier onlyUnderwriter(address _underwriter) {
        require(
            protocolRoles.isUnderwriter(_underwriter),
    ...
    modifier onlyRegisteredNetwork(address _network) {
        require(
            protocolRoles.isNetwork(_network),
    ...
    function createCreditLine(
        ...
        address _network
    ) external override onlyRequest onlyUnderwriter(_underwriter) onlyRegisteredNetwork(_network) {
    ...
    function depositCollateral(address _underwriter, uint256 _amount) public override {
    ...
    function stakeCollateral(
        ...
        require(
            deposits[_underwriter] >= _amount,
            "CreditManager: Not enough collateral deposited"
        );
        ...
        totalStakedCollateral += _amount;
    }

    function unstakeAndWithdrawCollateral(
        address _network,
        address _counterparty,
        address _underwriter,
        uint256 _amount
    ...
    function closeCreditLine(address _network, address _counterparty)
        external
     ...
     * View functions
     */
    function isValidLTV(address _network, address _counterparty)
        public
        view
        override
        returns (bool)
    ...
    function calculateLTV(address _network, address _counterparty)
        public
        view
        override
        returns (uint256)
    {
        ...
        uint256 creditLimitInCollateral = convertNetworkToCollateral(
            creditLine.network,
            creditLimit
        );
