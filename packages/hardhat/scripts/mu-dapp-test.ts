...
async function issueCreditLine() {
  try {
    ...
    if (
      Number(ethers.utils.formatEther(await mutualityToken.allowance(operator.address, underwriteManager.address))) == 0
    ) {
      await (
        await mutualityToken.connect(operator).approve(underwriteManager.address, ethers.utils.parseEther("1000000.0"))
      ).wait();
    }
