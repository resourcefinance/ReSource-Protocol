...
const mutualityAbi = "./deployments/localhost/MutualityToken.json";
...
async function issueCreditLine() {
  try {
    ...
    if (Number(ethers.utils.formatEther(await mutualityToken.allowance(operator.address, underwriteManager.address))) == 0) {
      await (await mutualityToken.connect(operator).approve(underwriteManager.address, ethers.utils.parseEther("1000000.0"))).wait();
    }
    if (Number(ethers.utils.formatEther(await mutualityToken.balanceOf(operator.address))) == 0) {      
      await (await mutualityToken.transfer(operator.address, ethers.utils.parseEther("100000.0"))).wait();
    }
    if (Number(ethers.utils.formatEther(await mutualityToken.balanceOf(operator.address))) > 0) {
      for (var member of members) {
        console.log(`Issuing $600 credit line to ${member.address}`);
        await (await underwriteManager.connect(operator).underwrite(rUSDAddress, ethers.utils.parseEther("600.0"), member.address)).wait();
      }
    }
