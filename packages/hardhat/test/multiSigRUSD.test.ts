...
describe("MultiSig Tests", function() {
  ...
  it("Successfully deploys a walletRegistry and multiSig", async function() {
    ...
    multiSigWallet = (await upgrades.deployProxy(walletFactory, [
      [client.address, guardian.address, coSigner.address],
      2,
    ])) as MultiSigWallet
  ...
  it("Successfully deploys RUSD and NetworkRegistry", async function() {
    ...
    networkRegistry = (await upgrades.deployProxy(networkRegistryFactory, [
      [multiSigWallet.address, targetWallet.address],
      [],
      deployer.address,
    ])) as NetworkRegistry
