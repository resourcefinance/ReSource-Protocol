...
describe("UnderwriteManager Tests", function () {
  ...
  it("Successfully deploys a RUSD contract", async function () {
    ...
    rUSD = (await upgrades.deployProxy(rUSDFactory, [networkRegistry.address, 7, underwriteManager.address], {
  ...
  it("Successfully use memberA credit line and claim underwriterA reward", async function () {
    ...
    await expect((await underwriteManager.creditLines(underwriterA.address, memberA.address)).reward).to.equal(ethers.utils.parseEther("20"));
  });

  it("Successfully claims reward for underwriterA", async function () {
    await expect(underwriteManager.connect(underwriterB).claimRewards([memberA.address])).to.be.reverted;

    await expect(underwriteManager.connect(underwriterA).claimRewards([memberA.address])).to.emit(underwriteManager, "CreditLineRewardClaimed");
  ...
  it("Extend an additional 5,000 mu from underwriterA for memberA", async function () {
    ...
    ).to.emit(underwriteManager, "UpdateCreditLine");
