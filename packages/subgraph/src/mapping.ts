...
export function handleNewCreditLine(event: NewCreditLine): void {
  let id = event.params.creditLine.underwritee.toHex() + "_" + event.params.creditLine.underwriter.toHex();
  ...
  creditLine.reward = event.params.creditLine.data.reward;
  creditLine.totalRewards = new BigInt(0);
  creditLine.save();
}
...
export function handleCreditLineReward(event: CreditLineReward): void {
  ...
  creditLine.reward = event.params.creditLine.creditLine.reward;
  creditLine.totalRewards = creditLine.totalRewards.plus(event.params.creditLine.creditLine.reward);
  creditLine.save();
}
...
export function handleCreditLineRewardClaimed(event: CreditLineRewardClaimed): void {
  ...
  for (var i = 0; i < creditLines.length; i++) {
    ...
    creditLine.reward = creditLines[i].creditLine.reward;
    creditLine.save();
  }
}
