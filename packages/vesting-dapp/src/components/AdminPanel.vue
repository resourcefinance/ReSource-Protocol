      ...
      <v-row>
        <v-col cols="12" md="4">
          <v-card class="mx-2">
            ...
            <v-card-text>
              <h1>
                {{ data.ownerBalance }}$ <small>{{ erc20.symbol }}</small>
              </h1>
            </v-card-text>
          </v-card>
        </v-col>
  ...
  methods: {
    ...
    async loadData() {
      ...
      this.data.vestingContractBalanceWei = await this.erc20.contract.methods
        ...
        .call()
      this.data.vestingContractBalance = toEth(this.data.vestingContractBalanceWei)
      this.fetchingDataCompleted = true
    },
    fund() {
      const amount = ethers.utils.parseUnits(this.form.amountToFund, "ether").toString()
