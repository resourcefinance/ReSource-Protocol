...
async function onWeb3Ready(web3) {
  ...
  if (validateEthAddress(store.state.globalConfig.tokenVestingContractAddress)) {
    ...
    store.state.isOwner = owner.toUpperCase() === window.ethereum.selectedAddress.toUpperCase()
    console.log(`Connected address is owner: ${store.state.isOwner}`)
    store.state.erc20.address = await store.state.tokenVesting.methods.getToken().call()
  }
  if (validateEthAddress(store.state.erc20.address)) {
    ...
    store.state.erc20.name = await store.state.erc20.contract.methods.name().call()
    store.state.erc20.symbol = await store.state.erc20.contract.methods.symbol().call()
  }
  new Vue({
    vuetify,
    router,
