...
describe("RestrictedCreditPool & Rewards Tests", function () {
  ...
  it("Ability to stake locked tokens and only receive unrestricted rewards", async function () {
    const ERC20Factory = await ethers.getContractFactory("MockERC20")
    const rewardToken = (await ERC20Factory.deploy(
      ethers.utils.parseEther("100000000")
    )) as MockERC20

    await (await contracts.creditPool.addRestriction(rewardToken.address)).wait()

    await (await contracts.creditPool.addRestriction(member.address)).wait()
    ...
    await (
      await contracts.sourceToken.transfer(member2.address, ethers.utils.parseEther("100"))
    ).wait()
    ...
    await (await contracts.creditPool.connect(member).stake(lockedbalBefore)).wait()
    await (await contracts.creditPool.connect(member2).stake(ethers.utils.parseEther("100"))).wait()

    const totalSupply = formatEther(await contracts.creditPool.totalSupply())
    const amountStaked = formatEther(lockedbalBefore.add(ethers.utils.parseEther("100")))
    expect(totalSupply).to.equal(amountStaked)

    await advanceTime(3600)

    const sourceEarned1 = await contracts.creditPool.earned(
      member.address,
      contracts.sourceToken.address
    )
    const sourceEarned2 = await contracts.creditPool.earned(
      member2.address,
      contracts.sourceToken.address
    )

    const mockEarned1 = await contracts.creditPool.earned(member.address, rewardToken.address)
    const mockEarned2 = await contracts.creditPool.earned(member2.address, rewardToken.address)

    expect(sourceEarned1).to.be.above(0)
    expect(sourceEarned2).to.be.above(0)
    expect(mockEarned1).to.equal(ethers.utils.parseEther("0"))
    expect(mockEarned2).to.be.above(0)

    await (await contracts.creditPool.connect(member).exit()).wait()
    const balAfter = await contracts.sourceToken.balanceOf(member.address)
    expect(floorEth(balAfter)).to.equal(floorEth(sourceEarned1))
